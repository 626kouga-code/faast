...
- **リスト内**:
  - リストタイトル（クリックで編集可）、優先度順並べ替えボタン、削除ボタンをヘッダーに配置する
  - 優先度順並べ替えボタンを押すと、そのリスト内のカード表示のみ優先度（高→中→低→未設定）順に切り替わる（表示上の並べ替えのみで、カードのposition自体は変更・保存されない。もう一度押すと元のドラッグ&ドロップ順に戻る）
